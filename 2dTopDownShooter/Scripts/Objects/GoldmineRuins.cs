...
public partial class GoldmineRuins : Node2D
{
	...
	{
		if (!body.IsInGroup(GameConstants.PlayerGroup) || _state == GoldmineState.Built)
			return;
	...
	private void OnPlayerEntered()
	{
		...
		if (_state == GoldmineState.Destroyed)
			StartBuilding();
		else if (_state == GoldmineState.Building)
	...
	{
		if (!_playerInRange || _state != GoldmineState.Building)
			return;
