...
public partial class MonastaryRuins : Node2D
{
	...
	private enum MonastaryState
	{
		Destroyed,
		Building,
		Built
	}
	private MonastaryState _state = MonastaryState.Destroyed;

	public bool IsBuilt => _state == MonastaryState.Built;
	...
	{
		if (!body.IsInGroup(GameConstants.PlayerGroup) || _state == MonastaryState.Built)
			return;
	...
	private void OnPlayerEntered()
	{
		...
		if (_state == MonastaryState.Destroyed)
			StartBuilding();
		else if (_state == MonastaryState.Building)
	...
	{
		_state = MonastaryState.Building;
		_sprite.Texture = ConstructionTexture;
		_buildTimer.Start();
	}
	private void OnBuildTick()
	{
		if (!_playerInRange || _state != MonastaryState.Building)
			return;
