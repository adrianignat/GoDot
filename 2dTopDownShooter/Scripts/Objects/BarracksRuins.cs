...
public partial class BarracksRuins : Node2D
{
	...
	private enum BarracksState
	{
		Destroyed,
		Building,
		Built
	}
	private BarracksState _state = BarracksState.Destroyed;

	public bool IsBuilt => _state == BarracksState.Built;
	...
	private void OnPlayerEntered()
	{
		...
		if (_state == BarracksState.Destroyed)
			StartBuilding();
		else if (_state == BarracksState.Building)
	...
	{
		_state = BarracksState.Building;
		_sprite.Texture = ConstructionTexture;
		_buildTimer.Start();
	}
	private void OnBuildTick()
	{
		if (!_playerInRange || _state != BarracksState.Building)
			return;
